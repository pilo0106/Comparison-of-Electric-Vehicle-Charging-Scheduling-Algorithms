...
class ChargingEnvESTWithLimit:
    ...
    def find_est(self, vehicle_idx):
        ...
        for station_idx in range(self.num_stations):
            ...
            for pile_idx in range(len(self.station_schedules[station_idx])):
                ...
                load_penalty = max(0, len(schedule) - 10) * 5  # 保留原有的負載懲罰
                
                # EST 核心：最小化等待時間
                if wait_time + load_penalty < min_wait_time:
                    min_wait_time = wait_time + load_penalty
    ...
    def schedule_vehicles(self):
        ...
        for vehicle_idx in vehicle_order:
            station_idx, pile_idx, start_time, finish_time, arrival_time = self.find_est(vehicle_idx)
...
if __name__ == "__main__":
    ...
    env = ChargingEnvESTWithLimit(num_vehicles, num_stations, station_piles, start_times, 
                                 battery_capacities, charging_rates, vehicle_positions, station_positions)
